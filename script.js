...
(function () {
    ...
    en: {
      ...
      preview: {
        ...
        reports: {
          ...
          chartTitle: "Sales overview — this week",
      ...
      chaos: {
        ...
        desc: "The same five things every supermarket manages — just organized differently.",
        before: "Before Suqiure",
        after: "With Suqiure"
      },
      footer: {
    ...
    ar: {
      ...
      preview: {
        ...
        reports: {
          ...
          chartTitle: "نظرة عامة على المبيعات — هذا الأسبوع",
          rankingTitle: "الأكثر مبيعًا",
          sold: "قطعة مباعة"
        },
        purchases: {
          colSupplier: "المورّد",
      ...
      chaos: {
        title: "من الفوضى إلى التحكم الكامل.",
        desc: "نفس العناصر الخمسة التي يديرها كل سوبرماركت، لكن بتنظيم مختلف تمامًا.",
        before: "قبل Suqiure",
        after: "مع Suqiure"
      },
      footer: {
  ...
  const demoRanking = [
    { productIndex: 0, sold: 182, pct: 100 },
    { productIndex: 1, sold: 145, pct: 80 },
    { productIndex: 5, sold: 98, pct: 54 },
    { productIndex: 2, sold: 61, pct: 34 }
  ];
  ...
  window.addEventListener('scroll', function () {
    ...
    if (window.scrollY > 80) navEl.classList.add('compact');
    else navEl.classList.remove('compact');
  }, { passive: true });
  ...
  /* ---- Count-up numbers (hero stats, report KPIs) ---- */
  ...
  function animateCounter(el) {
    ...
    function frame(now) {
      ...
      el.textContent = value.toLocaleString(currentLang === 'ar' ? 'ar-DZ' : 'en-US') + suffix;
  ...
  if (heroCounters.length && 'IntersectionObserver' in window) {
    ...
    heroCounters.forEach(function (el) { counterIo.observe(el); });
  } else {
  ...
  function switchScene(targetId) {
    const next = document.getElementById('scene-' + targetId);
    ...
    if (current) {
      ...
      setTimeout(function () { current.classList.remove('scene-leaving'); }, 200);
    }
    next.classList.add('active');

    if (targetId === 'reports') animateLineChart();
  }
  ...
  previewTabs.forEach(function (tab) {
    tab.addEventListener('click', function () {
      ...
      switchScene(tab.getAttribute('data-scene'));
    });
  });
  ...
  /* ------------------------------------------------------------------ */
  /* 11. Reports scene — animated SVG line chart + top-products ranking   */
  /* ------------------------------------------------------------------ */

  const weeklySales = [45, 60, 52, 78, 66, 92, 84]; // percentages, demo data only
  let lineChartPlayed = false;
  ...
  function buildLineChart() {
    ...
    const dotsGroup = document.getElementById('lineDots');
    if (!path || !area || !dotsGroup) return;

    const width = 560, height = 170, padTop = 15, padBottom = 15;
    const chartH = height - padTop - padBottom;
    const stepX = width / (weeklySales.length - 1);

    const points = weeklySales.map(function (v, i) {
      const x = i * stepX;
      const y = padTop + (1 - v / 100) * chartH;
      return [x, y];
    });

    const linePoints = points.map(function (p) { return p[0] + ',' + p[1]; }).join(' L ');
    const lineD = 'M ' + linePoints;
    const areaD = 'M ' + points[0][0] + ',' + height + ' L ' + linePoints + ' L ' + points[points.length - 1][0] + ',' + height + ' Z';

    path.setAttribute('d', lineD);
    area.setAttribute('d', areaD);

    dotsGroup.innerHTML = points.map(function (p) {
      return '<circle class="line-dot" cx="' + p[0] + '" cy="' + p[1] + '" r="4.5"></circle>';
    }).join('');
  }
  ...
  function animateLineChart() {
    ...
    document.querySelectorAll('.line-dot').forEach(function (d) { d.classList.remove('shown'); });

    // Force reflow so the transition re-triggers every time the tab opens.
    void path.getBoundingClientRect();

    requestAnimationFrame(function () {
      path.style.transition = 'stroke-dashoffset 1.1s ease';
      path.style.strokeDashoffset = '0';
      setTimeout(function () { area.classList.add('filled'); }, 400);
      document.querySelectorAll('.line-dot').forEach(function (dot, i) {
        setTimeout(function () { dot.classList.add('shown'); }, 200 + i * 90);
      });
    });

    document.querySelectorAll('#scene-reports .counter').forEach(animateCounter);
    renderRanking();
    lineChartPlayed = true;
  }

  function renderRanking() {
    const el = document.getElementById('rankingList');
    if (!el) return;
    el.innerHTML = demoRanking.map(function (row, i) {
      const p = demoProducts[row.productIndex];
      const name = currentLang === 'ar' ? p.ar : p.en;
      return (
        '<div class="ranking-item">' +
          '<div class="ranking-item-head">' +
            '<span class="rank-name"><span class="rank-num">' + (i + 1) + '</span>' + name + '</span>' +
            '<span class="rank-sold">' + row.sold + ' ' + t('preview.reports.sold') + '</span>' +
          '</div>' +
          '<div class="rank-bar"><i style="width:' + row.pct + '%"></i></div>' +
        '</div>'
      );
    }).join('');
  }
  ...
  /* ------------------------------------------------------------------ */
  /* 12b. From chaos to control — scattered chips settle into order       */
  /* ------------------------------------------------------------------ */

  const chaosStage = document.getElementById('chaosStage');
  if (chaosStage) {
    if ('IntersectionObserver' in window) {
      const chaosIo = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            chaosStage.classList.add('organized');
            chaosIo.disconnect();
          }
        });
      }, { threshold: 0.45 });
      chaosIo.observe(chaosStage);
    } else {
      chaosStage.classList.add('organized');
    }
  }
  ...
  function renderScreen(key) {
    if (!screenHost) { currentScreen = key; return; }

    if (!screenInitialized) {
      screenInitialized = true;
      currentScreen = key;
      screenHost.innerHTML = screenTemplate(key);
      return;
    }

    screenHost.classList.add('scene-leaving');
    setTimeout(function () {
      currentScreen = key;
      screenHost.innerHTML = screenTemplate(key);
      screenHost.classList.remove('scene-leaving');
      screenHost.classList.add('scene-enter');
      requestAnimationFrame(function () {
        requestAnimationFrame(function () { screenHost.classList.remove('scene-enter'); });
      });
    }, 160);
  }
  ...
  if (downloadBtn) {
    const hasRealLink = DOWNLOAD_URL !== 'YOUR_DOWNLOAD_LINK';
    downloadBtn.setAttribute('href', hasRealLink ? DOWNLOAD_URL : '#download');
    ...
    downloadBtn.addEventListener('click', function (e) {
      ...
      const originalText = downloadBtnLabel ? downloadBtnLabel.textContent : '';
      downloadBtn.classList.add('preparing');
      if (downloadBtnLabel) downloadBtnLabel.textContent = t('download.preparing');

      setTimeout(function () {
        downloadBtn.classList.remove('preparing');
        if (downloadBtnLabel) downloadBtnLabel.textContent = originalText || t('download.cta');
  ...
  // One-time orchestrated hero entrance (logo → headline → copy → CTA → interface).
  const heroEl = document.querySelector('.hero');
  if (heroEl) {
    requestAnimationFrame(function () {
      requestAnimationFrame(function () { heroEl.classList.add('loaded'); });
    });
  }
